...
export default function GoogleSheetModal({ isOpen, onClose }: GoogleSheetModalProps) {
  ...
  try {
    ...
    // Auto-create header row matching the exact Google Sheet columns if empty
    ...
    sheet.appendRow([
      data["Your Name"] || data.name || '',
      data["Work Email"] || data.email || '',
      data["Channel URL or Phone"] || data.phoneOrUrl || '',
      data["Primary Video Style"] || data.niche || '',
      data["Project Details & Monthly Target"] || data.message || '',
